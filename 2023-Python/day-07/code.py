...
# -- Part 1 --------------------------------------------------------------------
def overall_strength(hand):
    hand_strength = [sum([13**hand.count(card) for card in set(hand)])]
    card_strengths = ["23456789TJQKA".index(card) for card in hand]
    return hand_strength + card_strengths

input.sort(key = lambda x: overall_strength(x[0]))
result1 = sum([(pos + 1) * int(x[1]) for pos, x in enumerate(input)])

# -- Part 2 --------------------------------------------------------------------
def overall_strength2(hand):
    ...
    hand_strength = [sum([13**count for count in card_counts])] 
    card_strengths = ["J23456789TQKA".index(card) for card in hand]
    return hand_strength + card_strengths 

input.sort(key = lambda x: overall_strength2(x[0]))
